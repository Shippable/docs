page_main_title: Terraform CLI Overview
...
Job Images with Terraform CLI pre-installed are available for multiple operating systems, languages and versions. Each os-language-version combination is published in Docker hub as an image with a specific tag. The Docker hub account where we publish these images is `drydock`.
...
Terraform CLI is available on the drydock links specified below for these OS and language versions.
...
|----------|---------|-----------|
|0.8.7  | v5.7.3 and below  | Ubuntu 16.04 and Ubuntu 14.04 |
