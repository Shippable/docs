...
| `<NAME>`\_PATH 							| The path of the directory containing all the files for the resource. The actual source files
are checked out in the gitRepo subdirectory which can be accessed using the `<NAME>`\_STATE environment variable. |
| `<NAME>`\_STATE 							| The directory where the gitRepo is checked out. |
