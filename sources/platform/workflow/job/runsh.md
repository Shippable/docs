...
* **`runtime`** -- Optional. This is an object which can be used to configure
  the behavior of the job.
    ...
    * `timeoutMinutes` -- Optional. This sets the maximum time(in minutes) after which you want your runSh job to timeout. If this is not provided then timeout set at [Node Pool](/platform/management/subscription/node-pools/) level, Subscription level or default will be used, in that order. Default value for timeout is 60 minutes for free users and 120 minutes for paid users.
