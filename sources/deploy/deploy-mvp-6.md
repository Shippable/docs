page_main_title: Deploying a single container application with static image tags to a container orchestration service.
main_section: Deploy
sub_section: How To

# Deploying a single container application with static image tags to a container orchestration service.

This page will describe how you can use the [Shippable assembly lines platform](/platform/overview/) to deploy a single container application with static tags to a container orchestration service like Amazon ECS, GKE or Docker Cloud.

Shippable assumes that your images are versioned with unique names (we recommend tagging with $BRANCH.$BUILD_NUMBER). When your deploy job is triggered, it will deploy the latest version of the images if the image has changed (image tag) or docker options have changed.
