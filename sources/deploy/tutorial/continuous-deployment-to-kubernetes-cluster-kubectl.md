...
This tutorial explains how to continuously deploy any arbitrary Kubernetes cluster using native `kubectl` commands. It also assumes that you have working knowledge of Docker and Kubernetes and undestand the following concepts.
...
* Build and Push the Docker image of the application that you want to deploy to a registry of your choice. You could use our sample Node.js app in case you don't have one. [Docker Image](https://hub.docker.com/r/devopsrecipes/node_app/tags/) & [Push Docker image to Docker registry](/ci/push-docker-registry/)
* If your registry is a private registry, create a Kubernete Secret called `imagePullSecrets` to pull private images from a Docker registry. To learn more about, please read our blog [here](http://blog.shippable.com/kubernetes-tutorial-how-to-pull-private-docker-image-pod).

Create the secret called `private-registry-key` using `kubectl`.

`kubectl create secret docker-registry private-registry-key --docker-username="admin" --docker-password="XXXXXXXXX" --docker-email="<registered email address>" --docker-server="<server url>"`
...
If you do not have a kubeconfig file,
