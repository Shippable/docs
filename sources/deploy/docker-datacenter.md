page_main_title: Docker Datacenter: Deploy a Docker application to Docker Enterprise Edition
main_section: Deploy
...
* `op_cluster` is a **required** [cluster](/platform/workflow/resource/cluster/) resource that represents the Docker Datacenter cluster to which you're deploying the application.
...
resources:
  ...
  - name: app_image     # resource friendly name
    ...
    pointer:
      sourceName: devopsrecipes/deploy-ddc-basic    # replace with your image name
    seed:
...
* **Description:** `op_cluster` is a [cluster](/platform/workflow/resource/cluster/) resource that represents the  cluster on Docker Datacenter where your application is deployed to.
* **Required:** Yes.
* **Integrations needed:** Docker Datacenter integration
...
3. Add the following yml block to the existing `resources` section of your [shippable.yml](/platform/workflow/config/) file.
...
resources:
  ...
  - name: op_cluster    # resource friendly name
    ...
    pointer:
      sourceName: "app-ddc-cluster" # name of the actual cluster in Docker Datacenter
```
